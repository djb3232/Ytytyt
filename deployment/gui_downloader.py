    ...
    def build_command(self, action=None):
        ...
        if proxy:
            ...
            if youtube_urls:
                ...
                self.update_console(f"Finding the best proxy for {youtube_urls[0]}...")
                best_proxy = get_best_proxy(youtube_urls[0])
                
                if best_proxy:
                    self.update_console(f"Using best proxy: {best_proxy}")
